...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #Pick n random indexes with replacement from XX
        random_index = np.random.choice(len(XX), n, replace=True)
        #Split XX into X and y
        X_orig, y_orig = np.hsplit(np.array(XX), np.array([-1]))
        #Take bootstrapped samples and labels
        samples = X_orig[random_index].tolist()
        labels = y_orig[random_index].tolist()
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                votes2 = []
                for i in range(len(self.bootstraps_datasets)):
                    effective_vote2 = self.decision_trees[i].classify(record)
                    votes2.append(effective_vote2)
                y = np.append(y, np.argmax(np.bincount(votes2)))
